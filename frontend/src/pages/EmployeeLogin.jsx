...
export default function EmployeeLogin() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const { data } = await authAPI.employeeLogin(form);
      login(data.user);
    ...
    <div className="min-h-screen bg-gradient-to-br from-green-50 to-teal-100 flex items-center justify-center px-4">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-8">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-500 text-sm mt-1">View your profile & interview</p>
        </div>
        ...
        <div className="mt-6 text-center space-y-2">
          <p className="text-sm text-gray-500">
            ...
            <Link to="/register/employee" className="text-blue-600 hover:underline font-medium">Register</Link>
          </p>
          <p className="text-sm text-gray-500">
            Manager?{' '}
            <Link to="/login/manager" className="text-blue-600 hover:underline font-medium">Login here</Link>
          </p>
        </div>
      </div>
    </div>
